...
def _live_compose_ready() -> bool:
    return _docker_daemon_ready() and _compose_image_local()


@pytest.mark.skipif(
    not _live_compose_ready(),
    reason=f"docker compose live test needs daemon + local image {COMPOSE_IMAGE}",
)
